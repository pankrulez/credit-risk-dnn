    ...
    <div className="flex flex-col lg:flex-row gap-8 animate-in fade-in duration-500">
      ...
      <div className="w-full lg:w-2/3 order-2 lg:order-1">
        <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
          ...
          <section className="bg-slate-50 dark:bg-[#1c1b19] p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-zinc-800">
            <h3 className="text-base md:text-lg font-bold mb-4 text-slate-800 dark:text-slate-100">Client Profile</h3>
            {/* Fluid grid: 1 col on mobile, 2 on tablet, 4 on desktop */}
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-5">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">Limit Balance (NT$)</label>
          ...
          <section className="bg-slate-50 dark:bg-[#1c1b19] p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-zinc-800">
            ...
            {/* Fluid grid: 3 cols on mobile, 6 on desktop */}
              ...
              {[0, 2, 3, 4, 5, 6].map((m) => (
                <div key={`PAY_${m}`} className="space-y-1.5">
                  ...
                  <input type="number" name={`PAY_${m}`} value={(formData as any)[`PAY_${m}`]} onChange={handleChange} min="-2" max="8"
                    className="w-full px-3 py-2.5 md:py-2 text-base md:text-sm text-center rounded-xl border border-slate-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 focus:ring-2 focus:ring-teal-500 outline-none transition-all" />
                </div>
              ))}
            </div>
          </section>
          ...
          <button type="submit" disabled={loading}
            className="w-full md:w-auto px-8 py-3.5 md:py-3 bg-teal-600 hover:bg-teal-700 text-white font-bold rounded-xl shadow-md transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-base md:text-sm">
      ...
      <div className="w-full lg:w-1/3 order-1 lg:order-2">
        <div className="lg:sticky lg:top-6">
          ...
          {!result && !loading && !error && (
            <div className="p-8 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl text-center text-slate-500 bg-slate-50/50 dark:bg-[#1c1b19] hidden lg:block">
                  ...
                  <div className="space-y-2.5">
                    {Object.entries(result.topFeatures).slice(0, 3).map(([feature, weight], idx) => (
                      <div key={idx} className="flex justify-between items-center">
                        <span className="text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-zinc-800 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700">
                          {feature}
                        </span>
