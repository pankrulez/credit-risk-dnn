    ...
    <div className="flex flex-col lg:flex-row gap-8">
      ...
      <div className={`w-full lg:w-2/3 order-2 lg:order-1 transition-all duration-1000 ease-out delay-150 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <form onSubmit={handleSubmit} className="space-y-6 md:space-y-8">
          ...
          <section className="bg-slate-50 dark:bg-[#1c1b19] p-5 md:p-6 rounded-2xl border border-slate-200 dark:border-zinc-800 hover:shadow-md transition-shadow duration-300">
              ...
              {[0, 2, 3, 4, 5, 6].map((m) => (
                <div key={`PAY_${m}`} className="space-y-1.5">
                  ...
                  <input type="number" name={`PAY_${m}`} value={(formData as any)[`PAY_${m}`]} onChange={handleChange} min="-2" max="8"
                    className="w-full px-3 py-2.5 md:py-2 text-base md:text-sm text-center rounded-xl border border-slate-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 focus:ring-2 focus:ring-teal-500 outline-none transition-all hover:-translate-y-0.5" />
                </div>
              ))}
            </div>
          </section>
      ...
      <div className={`w-full lg:w-1/3 order-1 lg:order-2 transition-all duration-1000 ease-out delay-300 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
        <div className="lg:sticky lg:top-6">
          ...
          {!result && !loading && !error && (
            <div className="p-8 border border-dashed border-slate-300 dark:border-zinc-700 rounded-2xl text-center text-slate-500 bg-slate-50/50 dark:bg-[#1c1b19] hidden lg:block hover:bg-slate-100 dark:hover:bg-zinc-900 transition-colors duration-300">
          ...
          {result && !loading && (
            <div className={`relative p-6 rounded-2xl border shadow-lg overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 ${
              result.default ? 'border-red-300 bg-red-50 dark:border-red-900/50 dark:bg-red-950/20' : 'border-emerald-300 bg-emerald-50 dark:border-emerald-900/50 dark:bg-emerald-950/20'
            }`}>
              
              {/* Animated Glow in the result card */}
              <div className={`absolute inset-0 opacity-20 animate-gradient ${result.default ? 'bg-gradient-to-r from-red-500 to-orange-500' : 'bg-gradient-to-r from-emerald-500 to-teal-500'}`}></div>
                    ...
                    <div className="space-y-2.5">
                      {Object.entries(result.topFeatures).slice(0, 3).map(([feature, weight], idx) => (
                        <div key={idx} className="flex justify-between items-center group">
                          <span className="text-xs font-bold text-slate-700 dark:text-slate-200 bg-white dark:bg-zinc-800 px-2.5 py-1 rounded-md border border-slate-200 dark:border-slate-700 group-hover:border-teal-500 transition-colors">
                            {feature}
                          </span>
