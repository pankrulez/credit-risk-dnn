...
export const config = {
  // ── API ──────────────────────────────────────────────────────────────
  // In production: empty string → Next.js proxy route /api/proxy/*
  // In development: direct to localhost
  apiUrl:
  ...
  renderDocsUrl: requireEnv("NEXT_PUBLIC_RENDER_DOCS_URL"),
...
export function scaleFeatures(raw: number[]): number[] {
  ...
  if (!means.length || !stds.length) {
    console.warn("[Config] Feature means/stds missing — sending unscaled features");
    return raw;
  }
